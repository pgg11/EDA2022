...
class Pawn():
    ...
    def northMove(cls,board,pawns,side,lastPosition):
        for i in range(2,-1,-1):
            ...
            if board[row][col].botBorder:
                ...
                elif cls.checkForward(row,col,side,lastPosition):
                    return cls.moveForward(row,col,side)
        #Si ya no hay peones para avanzar busco moverse hacia atras o los costados o atras
        for i in range(2,-1,-1):
            row = pawns[i].posY
            col = pawns[i].posX
            #Si no hay una pared en el lado sur del cuadrado chequeo posibles movimientos
    ...
    def checkForward(cls,row,col,side,lastPosition):
        if side == 'N':
            return(not {'from_row':row+1,'from_col':col} in lastPosition)
        else:
            return(not {'from_row':row-1,'from_col':col} in lastPosition)
    ...
    def checkLeftSide(cls,board,row,col,lastPosition):
        if col > 0 and col != 8:
            return (board[row][col].leftBorder and board[row][col-1].value == ' ' and
              not {'from_row':row,'from_col':col-1} in lastPosition)
        #Cambio a agregar en el bot original
        elif col == 8:
